...
6. **WHEN** ユーザーが評価フォームを表示した **THEN** システムは入力フィールドを重要度順に配置する **AND** フィールドの表示順序は以下の通りである:
   1. 豆の名前（必須）- 最重要項目を最上部に配置
   2. 豆の産地
   3. 店名
   4. 焙煎度
   5. 評価スライダー（総合評価、酸味、苦味、香り）
...
- **フォームフィールドの優先順位**: 必須項目（豆の名前）を最上部に配置し、ユーザーが重要な情報から順に入力できるようにする（左上から右下への視線の流れを考慮）
